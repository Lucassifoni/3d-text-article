...
<script>
  ...
  import { render as renderToCanvas5 } from './renderToCanvas5';
  import FoldablePre from './../FoldablePre';

  const pointsToArr = points => points.map(p => [p.x, p.y]);

  export default {
    name: 'ParserSevenStep',
    components: {
      FoldablePre,
    },
    data() {
    ...
    methods: {
      parseAndDisplay() {
        ...
        const points = pointsToArr(convertToPoints(remapInstrs(parse(this.input))));
        this.result = points;//offset.data(points).offsetLine(10);
        this.draw();
      },
      draw() {
        ...
        const offset = new Offset();
        const points = pointsToArr(convertToPoints(remapInstrs(parse(this.input))));
        const offsetedPoints = offset.data(points).offsetLine(3)[0];
        renderToCanvas5(offsetedPoints.map(p => ({ x: p[0] * 6 - 100, y: p[1] * 6 - 100})), d);
        this.$options.$context2.globalCompositeOperation = 'multiply';
        this.$options.$context2.drawImage(this.$options.$canvas,0,0,300,300);
        this.$options.$context2.drawImage(this.$options.$canvas1,0,0,300,300);
      },
    },
    mounted() {
      this.$options.$canvas = this.$el.getElementsByTagName('canvas')[0];
      this.$options.$canvas1 = this.$el.getElementsByTagName('canvas')[1];
      this.$options.$canvas2 = this.$el.getElementsByTagName('canvas')[2];
      this.$options.$context = this.$options.$canvas.getContext('2d');
      this.$options.$context1 = this.$options.$canvas1.getContext('2d');
      this.$options.$context2 = this.$options.$canvas2.getContext('2d');
